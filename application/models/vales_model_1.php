...
por lo tanto la logica de niveles se manejara de forma diferente
	...
	function meses_requisicion()
	{
		...
		$q="SELECT DATE_FORMAT(DATE_ADD( CURDATE(), INTERVAL -155 DAY),'%M') mes_nombre, DATE_FORMAT(DATE_ADD( CURDATE(), INTERVAL -155 DAY),'%Y%m') mes 
				UNION
	...
	{		$q="SELECT COUNT(mes)  as num FROM tcm_requisicion WHERE id_fuente_fondo = ".$id_fuente_fondo." AND id_seccion = ".$id_seccion." AND mes LIKE '".$mes."' AND estado !=0";
			$query=$this->db->query($q);		
			return $query->result();
	}
