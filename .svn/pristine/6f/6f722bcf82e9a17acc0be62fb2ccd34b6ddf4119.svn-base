...
(function($){ 
	$.fn.validacion = function(options) {
		...
		settings = $.extend({
			...
			equal: null /*Campo con el que tiene que tener el mismo valor*/
        }, options);
...
------------------------------------------------------------------------------------------------------------------------*/		
		...
		this.menL = function(){
			if(settings.lonMax!=null) {
				...
				/*Funcion para validar que sea igual a otro campo*/
		this.equalF = function(){

			if(settings.equal!=null) {
				var otroCampo= $(equal).val();
				console.log("otroCampo "+ otroCampo);
				if($objeto.val()!= otroCampo && $objeto.val()!="") {
					if(settings.men==null || settings.men=="") 
						to="No concide con "+settings.equal;
					else
						to=settings.men;
					equalRes = false;	
					$self.err();
...
------------------------------------------------------------------------------------------------------------------------*/		
		...
		this.cor = function() {
			...
			if(reqRes && alfRes && numRes && mayRes && menRes && mayLRes && menLRes && pattRes && equalRes) { 
				if(settings.verOk)
